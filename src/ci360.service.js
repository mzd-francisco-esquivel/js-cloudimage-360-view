    ...
    updateView(forceUpdate, viewers, hotspotConfigs) {
        ...
        if (reInitView) {
            ...
            if (this.transitionInProgress) {
                this.eventQueue.push({forceUpdate, viewers, hotspotConfigs});
                return;
            }
            ...
            var viewIndex = viewers.findIndex(function (view) {
                return view.id === container.id;
            });

            // Crear nuevo contenedor sin el innerBox y preparar el nuevo
            var newContainer = container.cloneNode(true);
            newContainer.className = newContainer.className.replace(" initialized", "");
            newContainer.style.position = "absolute";
            newContainer.style.top = 0;
            newContainer.style.left = 0;
            newContainer.style.width = "100%";
            newContainer.style.opacity = 0; // Comenzar con opacidad 0 para el fade in
            newContainer.style.zIndex = 1;  // Asegurarse de que esté encima del contenedor antiguo
            newContainer.classList.add("fade-container");

            // Eliminar el innerBox del nuevo contenedor
            var innerBox = newContainer.querySelector('.cloudimage-360-inner-box');
            if (innerBox) {
                newContainer.removeChild(innerBox);
            }

            // Insertar el nuevo contenedor detrás del antiguo
            oldContainer.parentNode.insertBefore(newContainer, oldContainer.nextSibling);

            // Aplicar fade in al nuevo contenedor
            requestAnimationFrame(function () {
                newContainer.style.opacity = 1;
            });

            // Esperar a que el nuevo contenedor se haya desvanecido completamente antes de ocultar el antiguo
            setTimeout(function () {
                // Aplicar fade out al contenedor antiguo
                oldContainer.classList.add("fade-container");
                oldContainer.style.transition = "opacity 500ms";
                oldContainer.style.opacity = 0;
                // Esperar medio segundo y luego eliminar el antiguo contenedor y su innerBox
                setTimeout(function () {
            ...
            }, 500); // Esperar a que el nuevo contenedor haya completado su fade in

            // Reemplazar en la lista de visualizadores
            return viewers.splice(viewIndex, 1, new CI360Viewer(newContainer));
        }
